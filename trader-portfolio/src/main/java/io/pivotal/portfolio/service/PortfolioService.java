...
import io.pivotal.portfolio.config.ServiceProperties;
...
public class PortfolioService {
	...
	private Portfolio createPortfolio(Portfolio portfolio, List<Order> orders) {
		...
		portfolio.getHoldings().values().stream().forEach(this::refreshHolding);
	...
	private Quote getQuote(String symbol) {
		...
		Quote quote = restTemplate.getForObject( serviceProperties.getProtocol() + serviceProperties.getQuoteService() +
                "/quote/{symbol}", Quote.class, symbol);
		return quote;
	}
	...
	public Order addOrder(Order order) {
		...
		if (order.getOrderType() == OrderType.BUY) {
			...
			ResponseEntity<Double> result= restTemplate.getForEntity( serviceProperties.getProtocol() +
                    serviceProperties.getAccountService() + "/accounts/{userid}/decreaseBalance/{amount}", Double.class,
                    order.getAccountId(), amount);
            return handleResponse(order, result, "decreasing");
        } else {
			double amount = order.getQuantity()*order.getPrice().doubleValue()-order.getOrderFee().doubleValue();
			ResponseEntity<Double> result= restTemplate.getForEntity( serviceProperties.getProtocol() +
                    serviceProperties.getAccountService() + "/accounts/{userid}/increaseBalance/{amount}", Double.class,
                    order.getAccountId(), amount);
            return handleResponse(order, result, "increasing");
        }
	}

    private Order handleResponse(Order order, ResponseEntity<Double> result, String direction) {
        if (result.getStatusCode() == HttpStatus.OK) {
            ...
            logger.warn("PortfolioService:addOrder - " + direction + " balance HTTP not ok: ");
            return null;
        }
    }
}
