...
public class PortfolioServiceTest {
	...
	public void doGetPortfolio() {
        ...
        when( serviceProperties.getProtocol() ).thenReturn( "http://" );
        when( serviceProperties.getQuoteService() ).thenReturn( ServiceProperties.QUOTE_SERVICE );
	...
	public void doSaveOrder() {
		...
		double amount = ServiceTestConfiguration.order().getQuantity()*
                ServiceTestConfiguration.order().getPrice().doubleValue()+
                ServiceTestConfiguration.order().getOrderFee().doubleValue();
		ResponseEntity<Double> response = new ResponseEntity<Double>(100d, HttpStatus.OK);

        when( serviceProperties.getProtocol() ).thenReturn( "http://" );
        when( serviceProperties.getAccountService() ).thenReturn( ServiceProperties.ACCOUNT_SERVICE );
		when( restTemplate.getForEntity( "http://account-service/accounts/{userid}/decreaseBalance/{amount}",
                Double.class, ServiceTestConfiguration.order().getAccountId(), amount )).thenReturn(response);
